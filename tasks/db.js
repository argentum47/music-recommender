...
db.serialize(function () {
  ...
  var songStmt   = db.prepare("INSERT OR IGNORE INTO songs(name, artist_id, album_id, year) VALUES(?, ?, ?, ?)");
  ...
  ["numb", "breaking the habit", "faint", "somewhere i belong"].forEach(song => songStmt.run(song, 1, 1, 2003));
  ["crawling", "in the end"].forEach(song => songStmt.run(song, 1, 2, 2000));
  ["lost in the echo", "burn it down"].forEach(song => songStmt.run(song, 1, 3, 2012));
});